...
type Shovel struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.info) || (has(self.initProvider) && has(self.initProvider.info))",message="spec.forProvider.info is a required parameter"
